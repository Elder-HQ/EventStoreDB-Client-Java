...
class ReadSubscription implements Subscription {
    private final Subscriber<? super ResolvedEventWithContext> subscriber;
    ...
    ReadSubscription(Subscriber<? super ResolvedEventWithContext> subscriber) {
    ...
    public void onNext(long firstStreamPosition, long lastStreamPosition, Position lastAllStreamPosition, ResolvedEvent event) {
        lock.lock();
        while (requested.get() == 0 && !terminated.get()) {
            hasRequested.awaitUninterruptibly();
        }
        if (!terminated.get()) {
            subscriber.onNext(new ResolvedEventWithContext(firstStreamPosition, lastStreamPosition, lastAllStreamPosition, event));
